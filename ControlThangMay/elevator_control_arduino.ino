...
// ================== Tham s? h? th?ng - T?i ?u cho Realtime ==================
const uint8_t  NUM_FLOORS             = 6;
const uint16_t DOOR_OPEN_TIME         = 1200;
const uint16_t DOOR_CLOSE_TIME        = 800;
const uint16_t STEP_MS                = 800;   // Gi?m t? 1000ms xu?ng 800ms
const uint16_t POLL_MS                = 10;    // Gi?m t? 20ms xu?ng 10ms cho realtime
const uint16_t DEBOUNCE_MS            = 15;    // Gi?m t? 25ms xu?ng 15ms
const uint16_t WAIT_OPEN_MS           = 2000;
const uint16_t WAIT_AFTER_CLOSE_MS    = 1000;  // Gi?m t? 1500ms xu?ng 1000ms
const uint16_t ARRIVED_MS             = 500;   // Gi?m t? 1000ms xu?ng 500ms
const uint16_t SNAPSHOT_HEARTBEAT_MS  = 100;   // Gi?m t? 300ms xu?ng 100ms cho update nhanh h?n
...
struct DebounceGroup {
  ...
  bool hasChanged(uint8_t i, bool* previous) {
    bool current = stable[i];
    bool changed = (current != previous[i]);
    previous[i] = current;
    return changed;
  }
};
...
void markStateChanged(Car &c) {
  c.stateChanged = true;
}

void checkStateChanges(Car &c, uint32_t now) {
  if (c.floor != c.lastFloor || c.st != c.lastState || c.dir != c.lastDir) {
    sendStateEventCSV(c.name, c.floor, (int8_t)c.dir, (uint8_t)c.st, now);
    c.lastFloor = c.floor;
    c.lastState = c.st;
    c.lastDir = c.dir;
  }
...
void loopCar(Car &c, uint32_t now) {
  CarState oldState = c.st;
  uint8_t oldFloor = c.floor;
  Dir oldDir = c.dir;
  ...
  // Ki?m tra thay ??i và g?i event ngay l?p t?c
  if (c.floor != oldFloor || c.st != oldState || c.dir != oldDir) {
    checkStateChanges(c, now);
  }
}
...
static void sendBtnEventCSV(char grp, uint8_t idx, bool pressed, uint32_t t) {
  ...
  Serial.flush(); // Force send ngay l?p t?c
}
...
void setup() {
  ...
  // T?i ?u hóa Serial cho realtime
  Serial.setTimeout(50); // Gi?m timeout
...
void loop() {
  ...
  // 0) X? lý l?nh t? PC v?i ?? ?u tiên cao
  ...
  // 1) Debounce - t?n su?t cao cho responsive
  ...
  // 8) state machine 2 cabin - v?i state change tracking
  loopCar(A, now);
  loopCar(B, now);

  // 9) phát event state khi ??i - ?ã ???c handle trong loopCar
  // publishIfChanged(now); // Không c?n n?a vì ?ã handle realtime

  // 10) C?p nh?t LCD ~100ms thay vì 200ms
  ...
  // 11) CSV snapshot khi ??i ho?c heartbeat - t?n su?t cao h?n
